...
//            myUtils.mixin(createOptions, extraCreateOptions);
            ...
            Object.keys(props).forEach(function(x) {
                createOptions.Env.push(x + '=' + props[x]);
            });
...
//            myUtils.mixin(startOptions, extraStartOptions);
            ...
            volumes.forEach(function(x) {
                startOptions.Binds.push(x.source + ':' + x.destination);
            });

            // Network
            
            var firstNetwork = (Array.isArray(spec.env.networks) ? spec.env.networks[0] :
                                null);
            startOptions.NetworkMode = (firstNetwork ? $._.$[firstNetwork].getName() :
                                        'bridge');
            startOptions.PortBindings = (startOptions.PortBindings ?
            ...
            ports.forEach(function(x) {
                ...
                startOptions.PortBindings['' + x.containerPort + '/tcp'] =
                ...
                waitForRunning(container, function(err) {
                    if (err) {
                        ...
                        if (Array.isArray(spec.env.networks)) {
                            var dropFirst = spec.env.networks.slice(1);//1st already connected
